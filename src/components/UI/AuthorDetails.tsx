...
export const AuthorDetails: React.FC<PostCardProps> = ({ author }) => {
  ...
  ); // State to track followers count

  const handleFollowToggle = () => {
    setIsFollowing((prev) => !prev); // Toggle follow state

    if (isFollowing) {
      setFollowersCount((prev) => prev - 1); // Decrease follower count if unfollowed
    } else {
      setFollowersCount((prev) => prev + 1); // Increase follower count if followed
    }
  ...
  return (
    <div className="rounded-lg max-w-sm bg-white shadow-md mt-10 p-4">
      ...
      >
        ...
        {/* Change button text based on follow state */}
      </Button>
    </div>
  );
};
